    ...
    def move_astar(self, target, entities, game_map):
        ...
        for y1 in range(game_map.height):
            for x1 in range(game_map.width):
                libtcodpy.map_set_properties(fov, x1, y1, not game_map.tiles[x1][y1].block_sight,
        ...
        # It makes sense to keep path size relatively low to keep the monsters from running the map if there's an alternate path really far away
        if not libtcodpy.path_is_empty(my_path) and libtcodpy.path_size(my_path) < 25:
            ...
            # Keep the old move function as a backup so that if there are no paths (for example another monster blocks a corrider)
